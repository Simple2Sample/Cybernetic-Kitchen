...
def minste_avstand(strenger):
    # START IKKE-UTDELT KODE
    totaleAvstander = [0 for i in strenger]
    for x in range(len(strenger)):
        for y in range(x):
            xyAvstand = avstand(strenger[x], strenger[y])
            totaleAvstander[x] += xyAvstand
            totaleAvstander[y] += xyAvstand

    return min(totaleAvstander)
    # SLUTT IKKE-UTDELT KODE
